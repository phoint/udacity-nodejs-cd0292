...
import { imagesDir, resize, search, thumbDir } from "../../utilities/fileHandler"
...
describe("Test Resize Image", () => {
    ...
    it("resizing should throw error when width is not defined", () => {
        expectAsync(resize(trueInputPath, output, undefined, height)).toBeRejected();
    });

    it("resizing should return output path when provide all arguments", () => {
        expectAsync(resize(trueInputPath, output, width, height)).toBeResolvedTo(output);
    });
